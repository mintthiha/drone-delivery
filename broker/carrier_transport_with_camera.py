...
def read_qr_code(frame):
    ...
    for qr_code in qr_codes:
        ...
        last_word = qr_data.split('/')[-1]

        # Display the QR code data and type
        text = f"Data: {last_word} ({qr_type})"
        cv2.putText(frame, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        if (last_word == 'UPC_1' or last_word =='UPC_2' or last_word == 'UPC_3'):
            return last_word
    ...
    if newkeydata:
        ...
        if verify(signature, message_data, local_public_key, token):
            ...
            if current_state[UPC_ID]["status"] == 'waiting':
                ...
                print(f"Scan {UPC_ID} QRCODE to proceed")
                while True:
                    frame = picam2.capture_array()  # Capture the image

                    lastWord = read_qr_code(frame)             
                    
                    if (lastWord == UPC_ID):
                        print("Scan Successful")
                        break
                    if lastWord in ['UPC_1', 'UPC_2', 'UPC_3']:
                        if lastWord != UPC_ID:
                            print("Scanned wrong QR CODE try again")
                response = requests.post(f"{FLASK_API_URL}/{data.get('UPC_ID')}", json={"message": formatted_data, "token": carrier_tokens[UPC_ID]})
                if response.status_code == 201:
                ...
                picam2.capture_file(f"./proofs/{UPC_ID}.jpg")
                formatted_data = {
                    "UPC_ID": data.get("UPC_ID"),
                    "proof": {
                        "image": f"./proofs/{UPC_ID}.jpg"
                    },
                    "time": data.get("time"),
                    "status": current_state[UPC_ID]["status"]  
                }
